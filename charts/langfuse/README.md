...
![Version: 2.2.0](https://img.shields.io/badge/Version-2.2.0-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 4.35.0](https://img.shields.io/badge/AppVersion-4.35.0-informational?style=flat-square)
...
| dataRetention.clickhouse.clusterName | string | `"default"` | Cluster name used for `ON CLUSTER` application-table ALTERs when `clickhouse.cluster.enabled` is set. Set to "" to disable ON CLUSTER. |
...
| dataRetention.minio.image.repository | string | `"quay.io/minio/mc"` | Image for the object-storage retention jobs. Pulled from quay.io: the Docker Hub `minio/mc` repository is no longer publicly pullable. NOTE: scripts are written for POSIX sh (mc has no bash/sed/awk). |
...
| dataRetention.minio.lifecycle.enabled | bool | `true` | Apply per-prefix ILM expiry rules for raw event folders (`{projectId}/{type}/`) and `otel/`. Media objects (plain files at `{projectId}/{mediaId}`) are never matched. Only rendered for the bundled object store or an explicit S3-compatible endpoint (`s3.storageProvider: s3`). |
